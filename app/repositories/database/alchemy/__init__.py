...
from entities import BaseEntity, ArticleEntity, ScoreEntity
from repositories.database import AbstractRepo, AbstractScoreRepo, AbstractArticleRepo
...
class AlchemyRepo(AbstractRepo):
    ...
    @session_maker
    def delete(self, session: Session, pk_key: UUID) -> None:
        sq = delete(self.entity).where(self.entity.id == id)
        session.execute(sq)
        session.commit()


class ArticleRepo(AbstractArticleRepo, AlchemyRepo):
    entity = ArticleEntity


class ScoreRepo(AbstractScoreRepo, AlchemyRepo):
    entity = ScoreEntity
